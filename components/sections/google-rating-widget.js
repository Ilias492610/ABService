...
const GoogleRatingWidget = () => {
  ...
  return (
    <div className="inline-flex items-center gap-2 md:gap-4 rounded-xl bg-white p-2 md:p-4 shadow-[0_4px_14px_rgba(0,0,0,0.08)] relative">
      ...
      <div>
        <p className="text-xs md:text-sm font-bold text-[#1a2c3d]">AB Service</p>
        <div className="flex items-center gap-1 md:gap-2">
          <span className="text-xs md:text-sm font-bold text-[#1a2c3d]">5.0</span>
          <div className="flex text-[#f59e0b]">
            {Array.from({ length: 5 }).map((_, index) => (
              <span key={index} className="text-sm md:text-base leading-none">★</span>
            ))}
          </div>
        </div>
      </div>
